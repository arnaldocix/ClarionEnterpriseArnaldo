...
PDFGeneratorClass.GetFileName              PROCEDURE()
 CODE
           ...
           IF LOC:DC THEN
              LOC:OLDMAPMODE=SetMapMode(LOC:DC,1)
                LOC:HFont = CreateFont(|
                 -MulDiv(ABS(SELF.PointSize*10), GetDeviceCaps(LOC:DC, 90), 72), | !LOGPIXELSY = 90 Logical pixels inch in Y
                 0,|
                 0,|
                 0,|
              ...
              IF LOC:HFont THEN
                 IF SelectObject(LOC:DC, LOC:HFont) THEN
                    ...
                    RETURN SELF.PPIToDPI(LOC:CharSize.eW+(LOC:CharSize.eW/20))
                 END
                 DeleteObject(LOC:HFont)
              END
              LOC:OLDMAPMODE=SetMapMode(LOC:DC,LOC:OLDMAPMODE)
...
PDFGeneratorClass.PPIToDPI      PROCEDURE(LONG pValue)
LOC:DC     UNSIGNED, AUTO
LOC:DPI    REAL, AUTO
LOC:PPI    LONG, AUTO
 CODE
    ...
    LOC:DC = SELF.DC
    IF LOC:DC THEN    
       LOC:PPI = GetDeviceCaps(LOC:DC, 88) !LOGPIXELSX = 88 Logical pixels inch in X     
       LOC:DPI = LOC:PPI/96
    END
    RETURN (pValue / LOC:DPI)
     ...
     IF NOT ERRORCODE()
        ...
        IF NOT SELF.FontsNames.BuiltIn THEN
           ...
           IF LOC:DC THEN
              ...
              LOC:HFont = CreateFont( |
                 -MulDiv (750, GetDeviceCaps(LOC:DC, 90), 72), | !LOGPIXELSY = 90 Logical pixels inch in Y
                 0, |
                    ...
                    IF LOC:CSOUTLINETEXTMETRIC_SIZE AND LOC:CSOUTLINETEXTMETRIC_SIZE>=SIZE(OUTLINETEXTMETRIC) THEN
                       ...
                       IF GetOutlineTextMetrics(LOC:DC, LOC:CSOUTLINETEXTMETRIC_SIZE, LOC:CSOUTLINETEXTMETRIC)
                          ...
                          IF SELF.FontsNames.FirstChar = 0
                             ...
                             IF SELF.FontsNames.FirstChar = 0 AND SELF.FontsNames.LastChar <> 0
                                SELF.FontsNames.FirstChar = 1
                             END                             
                               ...
                               IF GetTextExtentPoint32(LOC:DC,LOC:CharToTest, 1,LOC:CharSize)<>0 THEN
                                  IF LOC:Subclasing THEN
                                     IF SELF.FontsNames.CharWidth[LOC:I] THEN
                                        SELF.FontsNames.CharWidth[LOC:I]=SELF.PPIToDPI(LOC:CharSize.eW)
                                     END
                                  ELSE
                                     SELF.FontsNames.CharWidth[LOC:I]=SELF.PPIToDPI(LOC:CharSize.eW)
                                  END
...
PDFReportGenerator.IReportGenerator.CloseDocument       PROCEDURE()
 CODE
     ...
     SELF.RemoveScanCopyFiles()
     RETURN Level:Benign

PDFReportGenerator.RemoveScanCopyFiles       PROCEDURE()
idx LONG,AUTO
 CODE
    LOOP idx=1 TO RECORDS(SELF.ScanCopyFiles)
         GET(SELF.ScanCopyFiles,idx)
         IF NOT ERRORCODE()
            REMOVE(SELF.ScanCopyFiles.FileName)
         END
   END
     
PDFReportGenerator.IReportGenerator.SupportPageProcessing PROCEDURE()
 CODE
    RETURN TRUE

PDFReportGenerator.IReportGenerator.SupportPageParsing  PROCEDURE()
 CODE
    IF SELF.GetScanCopyMode() = TRUE
       RETURN FALSE
    ELSE
       RETURN TRUE
    END

PDFReportGenerator.IReportGenerator.OpenPage PROCEDURE(STRING PageName)
 CODE
    RETURN Level:Benign

PDFReportGenerator.IReportGenerator.StartPageProcess            PROCEDURE(SHORT pBoxLeft,SHORT pBoxTop,SHORT pBoxRight,SHORT pBoxBottom,STRING pPageName)
...
H LONG,AUTO
 CODE
     ...
     IF SELF.GetScanCopyMode() = TRUE
        ...
        IF NOT SELF.ClaRunExt.ImageToPNG(lInPageName,lOutPageName, SELF.ScanCopyResolution)
           W = pBoxRight-pBoxLeft
           H = pBoxBottom-pBoxTop
           SELF.DrawImage(0,0,W,H,lOutPageName)
           SELF.ScanCopyFiles.FileName = lOutPageName
           ADD(SELF.ScanCopyFiles)
           RETURN Level:Benign
        END
        RETURN Level:Fatal     
     END
...
PDFReportGenerator.IReportGenerator.GetFileName              PROCEDURE()
 CODE
...
PDFReportGenerator.SetScanCopyMode         PROCEDURE(BYTE onOff)
 CODE
    IF onOff >0
       SELF.ScanCopyMode = TRUE
...
PDFReportGenerator.GetScanCopyMode         PROCEDURE()
 CODE
    RETURN SELF.ScanCopyMode

PDFReportGenerator.SetScanCopyResolution   PROCEDURE(USHORT imageRes)
 CODE
    IF imageRes > 2400
       SELF.ScanCopyResolution = 2400
    ELSE
       SELF.ScanCopyResolution = imageRes
    END
    
PDFReportGenerator.GetScanCopyResolution   PROCEDURE()
 CODE
